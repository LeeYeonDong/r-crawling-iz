...
times <- seq(from = 3, to = 5, by = 0.00001)
...
# for(n in 1:length(키워드_url)){
# remDr$navigate(키워드_url[n])
# 
# times <- seq(from = 1, to = 2, by = 0.00001)
# 
# # Scroll down to load all comments
# last_height <- (remDr$executeScript("return document.documentElement.scrollHeight"))[[1]][1]
# while (TRUE) {
...
링크_ytb <- mu_df_raw$링크_ytb %>% unique()


df_ytb_com <- tibble()
times <- seq(from = 3, to = 5, by = 0.00001)
times2 <- rep(c(0,0,0,0,5), each = length(링크_ytb))
...
for(i in 1:length(링크_ytb_c)){
  tryCatch({
    remDr$navigate(링크_ytb_c[i])
    Sys.sleep(4)
    pause <- remDr$findElement("css","button.ytp-play-button") 
    pause$clickElement()
    ...
    df_ytb_com_c <- bind_rows(df_ytb_com_c,df_ytb)
...
library(xlsx)
write.xlsx(df_ytb_com_c, "D:/대학원/상담/커뮤니케이션학과/의료분쟁/df_ytb_com3_c.xlsx")
