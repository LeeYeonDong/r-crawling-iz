...
for (i in 1:length(dd2021)){
  for (j in 1:length(seq)){
    navi_야구.tmp <- paste0(head,searchword,body0,body1,dd2021[i],body2,dd2021[i],body3,from2021[i],to,from2021[i],body5,seq[j],body6)
    navi_야구 <- append(navi_야구,navi_야구.tmp)
  }
}
...
## 네이버뉴스 링크 수집
for (i in 1:length(navi_야구)){
  tryCatch({
    cat(i, '페이지 수집 중', '입니다.\n') 
    
    body <- navi_야구[i] %>% read_html()
    ...
    if (length(링크_야구.tmp) != 0) {
      링크_야구 <- append(링크_야구,링크_야구.tmp)
    } else {
      링크_야구 <- append(링크_야구,"수동확인")
    }    
...
### news.naver 링크
news.naver_야구 <- grep("https://news.naver.com",링크_야구)
링크_news.naver_야구 <- 링크_야구[news.naver_야구]
...
for (i in 1:length(링크_news.naver_야구)){
  tryCatch({
    cat(i, '페이지 수집 중', '입니다.\n') 
    body <- 링크_news.naver_야구[i] %>% read_html()
...
### sports.news 링크
sports.news_야구 <- grep("https://sports.news",링크_야구)
링크_sports.news_야구 <- 링크_야구[sports.news_야구]
...
for (i in 1:length(링크_news.naver_야구)){
  tryCatch({
    remDr$navigate(링크_news.naver_야구[i])
    ...
    좋아_news.naver_야구 <- append(좋아_news.naver_야구, 반응_야구.tmp[1])
    훈훈_news.naver_야구 <- append(훈훈_news.naver_야구, 반응_야구.tmp[2])
    슬퍼_news.naver_야구 <- append(슬퍼_news.naver_야구, 반응_야구.tmp[3])
    화나_news.naver_야구 <- append(화나_news.naver_야구, 반응_야구.tmp[4])
    후속_news.naver_야구 <- append(후속_news.naver_야구, 반응_야구.tmp[5])
...
names(뉴스_news.naver_야구) <- c("언론사_야구","제목_야구","날짜_야구","링크_야구","좋아_야구","훈훈_야구","슬퍼_야구","화나_야구","후속_야구")
...
for (i in 1:length(링크_sports.news_야구)){
  tryCatch({
    remDr$navigate(링크_sports.news_야구[i])
    body <- remDr$getPageSource()[[1]]
    ...
    좋아_sports.news_야구 <- append(좋아_sports.news_야구, 반응_야구.tmp[1])
    훈훈_sports.news_야구 <- append(훈훈_sports.news_야구, 반응_야구.tmp[2])
    슬퍼_sports.news_야구 <- append(슬퍼_sports.news_야구, 반응_야구.tmp[3])
    화나_sports.news_야구 <- append(화나_sports.news_야구, 반응_야구.tmp[4])
    후속_sports.news_야구 <- append(후속_sports.news_야구, 반응_야구.tmp[5])
...
names(뉴스_sports.news_야구) <- c("언론사_야구","제목_야구","날짜_야구","링크_야구","좋아_야구","슬퍼_야구","화나_야구","팬이_야구","후속_야구")
...
## 데이터 프레임 저장
뉴스_df_야구 <- rbind(뉴스_news.naver_야구,뉴스_sports.news_야구)

write.csv(뉴스_news.naver_야구, file = "C:/대학원/논문/소논문/뉴스_news.naver_야구.csv", row.names=FALSE)

write.csv(뉴스_sports.news_야구, file = "C:/대학원/논문/소논문/뉴스_sports.news_야구.csv", row.names=FALSE)
